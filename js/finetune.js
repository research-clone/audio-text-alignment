...
function applyDefaults() {
    try {
        ...
        setButtonText("couple1", couple1, "ON", "OFF");
...
function previousAudioAndJson() {
    if (jsonFiles.length != audioFiles.length) {
        ...
        audioName = audioFile.name.slice(0, -4)
        if (audioFile) {
            audioProcess(audioFile)
        } else {
            alert("Error when loading audio");
        }
...
function audioProcess(audioFile) {
...
function prepareControls() {
    ...
    document.getElementById("couple1").addEventListener("click", changeCouple1);
    ...
    document.getElementById("load_previous_file").addEventListener("click", previousAudioAndJson);
    document.getElementById("load_next_file").addEventListener("click", nextAudioAndJson);
...
function downloadClick() {
    ...
    var suggestedFileName = "tuned_" + audioName + "." + outputFormat;
